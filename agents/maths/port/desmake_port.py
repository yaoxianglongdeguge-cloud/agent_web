...
def tools_make(myname:str,action:str,tool:str):
   ...
   if action == "c":
       ...
       ai_speak1,result=module.other_run_self(myname,out_input)
       print(ai_speak1,flush=True)

       for line in sys.stdin:
         out_input2 = line.strip()
         if not line:
          continue
         if out_input2=="quit":
           break
         
         ai_speak2,result=module.other_run_self(myname,out_input2)
         print(ai_speak2,flush=True)

       shutil.copytree(func_dir, stay_dir/"ai_tools"/"tools"/func_dir)
      
   elif action=="d":
       
        function_name=tool# 无后缀
